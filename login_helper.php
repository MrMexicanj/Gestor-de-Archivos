...
//esta esla parte puesta
function authenticate_user($username, $password) {
    global $USERS;
    foreach ($USERS as $user) {
        if ($user['username'] === $username && $user['passwordEncrypted'] === encrypt_password($password, $user['passwordSalt'])) {
            return true;
        }
    }
    return false;
...
function encrypt_password($password, $salt) {
    ...
    return strtoupper(hash("sha512", $passwordWithSalt));
}
